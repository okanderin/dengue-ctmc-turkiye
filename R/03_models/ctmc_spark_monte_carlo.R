...
#
# CORRECTED VERSION:
#   (1) beta_hv parameter added to lambda calculation
#   (2) ESTIMAND (Option 3): MAIN production runs use the deterministic-mean
#       P_est = P_est(E[EIP]) (single closed-form on mean lambda over EIP
#       draws), NOT the Jensen-corrected E[P_est(EIP)]. The Jensen-corrected
#       estimand and the Jensen deviation E[P_est(EIP)]-P_est(E[EIP]) are
#       produced separately by the deterministic reference runner
#       ctmc_spark.R (§6.1.2 Stage 2), where the deviation is shown to be
#       operationally negligible (mean |Δ|~0.002, max ~0.032). The split is
#       intentional (speed): do NOT re-add per-draw Jensen here.
#   (3) POISSON-THINNING monthly major probability:
#       p_month_major = 1 - exp(-lambda_import * P_est), replacing the
#       Bernoulli product q_import_month * P_est (which saturates and
#       undercounts multiple introductions). Uses lambda_import (full-month
#       expected imports), consistent with Eq. 5f. Independence across
#       introductions => STRUCTURAL UPPER BOUND.
# ==========================================================
...
compute_ctmc_month_mc <- function(district_id, year, month,
  ...
  # Double-counting AND unit-mismatch guard (same logic as ctmc_spark.R).
  # NOTE: include_import_in_q = TRUE adds lambda_import_month (expected imports
  # PER MONTH) directly to lambda_local_n, which are PER-DAY birth rates in the
  # continuous-time BD process. These units are inconsistent (month vs day), so
  # this option is UNSUPPORTED for production. The default FALSE path (import
  # handled downstream via Poisson thinning on lambda_import) is the correct
  # and only validated route; main results use FALSE and are unaffected.
  if (isTRUE(include_import_in_q) && lambda_import_month > 0) {
    warning(
      "include_import_in_q = TRUE mixes a MONTHLY import mean into PER-DAY BD ",
      "birth rates (unit mismatch) and double-counts importation. This option ",
      "is unsupported; use the default FALSE (Poisson-thinning on lambda_import).",
      call. = FALSE
    )
  }
  ...
  # ============================================================
  # DESIGN NOTE (Option 3 — deliberate, NOT a bug):
  # This Monte Carlo production runner uses the DETERMINISTIC-MEAN estimand
  #     P_est = P_est( E_EIP[lambda] )        [i.e. P_est(E[EIP])]
  # It evaluates make_lambda_local() WITHOUT return_draws (scalar mean lambda)
  # and runs the closed-form gambler's-ruin ONCE on that mean. This is the
  # fast path used to generate the rep{n_rep} horizon/yearly/monthly tables.
  #
  # The Jensen-CORRECTED estimand  E_EIP[ P_est(lambda(EIP)) ]  is computed
  # SEPARATELY, in the deterministic reference runner ctmc_spark.R (per-draw
  # averaging). The gap between the two — E[P_est(EIP)] - P_est(E[EIP]) — is
  # exactly the "Jensen deviation" quantified in thesis §6.1.2 (Stage 2),
  # where it is shown to be operationally negligible (mean |Δ| ~0.002,
  # max ~0.032). Because that gap is negligible, the two runners agree to
  # ~0.002 on P_est; this runner is preferred here purely for speed
  # (no inner per-draw extinction loop × n_rep).
  #
  # ⇒ Do NOT "re-add Jensen here to match ctmc_spark.R": the split is
  #   intentional. The thesis text (§5 methods, §6.1.2, Appendix A.2) must
  #   describe the MAIN runs as P_est(E[EIP]) with the Jensen deviation
  #   reported separately as negligible — NOT as Jensen-corrected.
  # ============================================================
  lam_i1 <- make_lambda_local(
    ...
    # return_draws omitted (= FALSE): returns scalar mean lambda over EIP draws
  )
  ...
  p_est <- 1 - q                        # P_est(E[EIP])  (deterministic-mean estimand)
  ...
  compute_one_rep <- function(rep_id) {
    res_month <- df %>%
      dplyr::mutate(
        ...
        # ----------------------------------------------------------------
        # POISSON-THINNING monthly major-outbreak probability (Eq. 5f').
        # ----------------------------------------------------------------
        # Introductions arrive as an (inhomogeneous) Poisson process with
        # monthly mean Lambda_import,m = lambda_import (expected imports/month,
        # already built on the FULL month length days_in_month, not a 3-day
        # window). By the thinning theorem, the introductions that go on to
        # establish are themselves Poisson with mean Lambda_import,m * P_est.
        # Hence P(>=1 major outbreak in month m):
        #     p_month_major = 1 - exp(-Lambda_import,m * p_local_major)
        # This correctly counts MULTIPLE independent introductions per month
        # and superposition across the season, unlike the earlier Bernoulli
        # product q_import_month * P_est, which saturates and undercounts in
        # high-Lambda units (e.g. Istanbul/Kartal). Independence across
        # introductions makes this a STRUCTURAL UPPER BOUND (see Discussion:
        # co-seasonal introductions share environmental/vector state).
        p_month_major = 1 - exp(
          -pmax(.data$lambda_import, 0) * pmin(pmax(.data$p_local_major, 0), 1)
        )
      ) %>%
      dplyr::select(-tmp) %>%
