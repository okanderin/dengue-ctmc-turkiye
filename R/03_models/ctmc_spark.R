...
compute_ctmc_month_spark <- function(temp_c, rh, lambda_import_month,
  ...
  #
  # Empirical (this run, all SSPs): concavity dominates in every active cell
  #   → E[P_est] < P_est(E[EIP]) throughout; no R_eff-regime sign reversal observed.
  #
...
run_ctmc_spark <- function(
  ...
  res <- df %>%
    ...
    dplyr::mutate(
      # ----------------------------------------------------------------
      # POISSON-THINNING aylık majör-salgın olasılığı (Denklem 5f').
      # ----------------------------------------------------------------
      # İthal girişler aylık ortalaması Lambda_import,m = lambda_import olan
      # (inhomojen) bir Poisson süreciyle gelir (lambda_import ayın TAM gün
      # sayısı days_in_month üzerinden kurulur; 3 günlük pencere DEĞİL).
      # İnceltme (thinning) teoremi gereği tutunmayı başaran girişler de
      # ortalaması Lambda_import,m * P_est olan bir Poisson sürecidir:
      #     p_month_major = 1 - exp(-Lambda_import,m * p_local_major)
      # Bu, ay içindeki BİRDEN FAZLA bağımsız girişi ve sezon boyu
      # süperpozisyonu doğru sayar; eski Bernoulli çarpımı
      # q_import_month * P_est yüksek-Lambda birimlerde (ör. İstanbul/Kartal)
      # doygunlaşıp riski olduğundan düşük gösterir. Girişler arası
      # bağımsızlık varsayımı bunu YAPISAL ÜST SINIR yapar (bkz. Tartışma).
      p_month_major = 1 - exp(
        -pmax(.data$lambda_import, 0) * pmin(pmax(.data$p_local_major, 0), 1)
      )
    )
  
  # ---- Yearly p(>=1 major outbreak) using Poisson-thinned months:
  # p_year = 1 - prod_m (1 - p_month_major) = 1 - exp(-Σ_m Λ_m·P_est,m)
  res_yearly <- res %>%
