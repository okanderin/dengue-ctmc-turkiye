...
#
# BUGFIX: the cache is now keyed by the trait table's EIP parameters (i.e.
# per species), NOT a single shared slot. Previously the first species to be
# processed wrote .eip_logn_fit_env$coef, and every later species reused those
# same sdlog(T) coefficients — silently mixing Ae. aegypti and Ae. albopictus
# EIP heterogeneity. The mean EIP was always correct (eip_mean_of_T reads the
# passed `tab`), so only the DISPERSION sdlog(T) was affected; but that still
# violated the species-specific EIP-variance claim. Keying by the eip_dev_rate
# row (form, c, T0, Tm) gives each species its own calibrated coefficients.
.eip_logn_cache_key <- function(tab) {
  r <- .get_trait_row(tab, "eip_dev_rate")
  paste("eip_dev_rate",
        tolower(as.character(r$form[[1]])),
        format(as.numeric(r$c[[1]]),  digits = 15),
        format(as.numeric(r$T0[[1]]), digits = 15),
        format(as.numeric(r$Tm[[1]]), digits = 15),
        sep = "|")
}

get_eip_logn_params <- function(T, tab, force_recalibrate = FALSE) {
  key <- .eip_logn_cache_key(tab)
  if (isTRUE(force_recalibrate) || is.null(.eip_logn_fit_env[[key]])) {
    .eip_logn_fit_env[[key]] <- .calibrate_eip_logn_sdlog(tab)
  }
  cf <- .eip_logn_fit_env[[key]]
