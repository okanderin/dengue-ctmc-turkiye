...
##
## VERSION 4 — METHOD 1: zone (province) RAW arrivals A_d, direct
##
## KEY CHANGE FROM v3
##   V_in_day now uses each sentinel's PROVINCE (zone) raw foreign
##   accommodation arrivals A_d directly, instead of allocating the
##   NATIONAL total across the 5 sentinels via a within-5-district
##   share (V_TR).
##
...
##               A_d = PROVINCE-total foreign accommodation arrivals
##                     (Σ over all districts of the sentinel's province;
##                      source: build_travel_weights_zone.R → `gelis_yabanci`
##                      = province total). This is a ZONE quantity, NOT the
##                      sentinel district's own arrivals.
##
##   LABELLING: because A_d is the province (zone) total, each sentinel is a
##   CLIMATE-ARCHETYPE representative of its province, not a district-specific
##   estimate. Kartal itself is residential (few foreign arrivals) but stands
##   for the İstanbul climate zone, whose other districts receive heavy
##   tourism. Report/plot these as e.g. "İstanbul (Kartal climate proxy)",
##   "Muğla (Fethiye climate proxy)", etc. — never as district-level risk.
##
...
imp_m <- grid %>%
  dplyr::mutate(
    ...
    # V_in_day: bu sentinel'in IL (zone) MUTLAK gunluk yabanci gelisi (METHOD 1)
    # A_d = il-zonu toplami (gelis_yabanci, build_travel_weights_zone.R); ilce
    # DEGIL. Ulusal toplam / normalizasyon YOK. Sentinel = il iklim temsilcisi.
    ...
    # Monthly totals
    # CANONICAL monthly importation intensity used by the CTMC spark model:
    # expected imported cases over the FULL month length (Eq. 5f-consistent).
    expected_imported_cases_per_month = lambda_import_per_day * days_in_month,

    # DIAGNOSTIC ONLY (NOT used by the model): Bernoulli P(>=1 introduction in
    # a d_days exposure window). Retained for backward-compat / reporting.
    # The monthly major-outbreak probability is computed downstream via
    # Poisson thinning as 1 - exp(-lambda_import * P_est), NOT q_import_month
    # * P_est. Do NOT reintroduce the q_import_month * P_est product: it
    # saturates and undercounts multiple introductions in high-Lambda units.
    lambda_import_window = lambda_import_per_day * pmin(d_days, days_in_month),
    q_import_month       = 1 - exp(-lambda_import_window),

    # Canonical monthly Poisson mean handed to the CTMC (expected imports/month)
